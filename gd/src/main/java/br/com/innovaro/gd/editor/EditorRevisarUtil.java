...
public final class EditorRevisarUtil extends VerticalLayout {
    ...
    public EditorRevisarUtil(final ReportEditorRevisarListener listener) {
    ...
    private HorizontalLayout criarCamposDatas() {
    	...
    	Button enviarAprovacao = new Button("Enviar para Aprovação");
	...
	public void janelaConfirmacao() {
    	...
    	Label text = new Label("Deseja enviar Documento para aprovação?");
    	...
    	sim.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				documento.setStatus(DocumentoStatusType.APROVACAO.getTitle());
